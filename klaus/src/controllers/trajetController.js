...
exports.searchTrajets = async (req, res) => {
  try {
    ...
    // Accepter les alias envoyés par différents clients front.
    if (!criteria.ville_depart && criteria.depart) {
      criteria.ville_depart = criteria.depart;
    }
    if (!criteria.ville_arrivee && criteria.destination) {
      criteria.ville_arrivee = criteria.destination;
    }
    if (!criteria.date_depart && criteria.date) {
      criteria.date_depart = criteria.date;
    }
 ...
 * @route   PUT /api/trajets/:id/complete
 * @access  Private (conducteur uniquement)
 */
exports.completeTrajet = async (req, res) => {
  try {
    const trajet = await Trajet.findById(req.params.id);
    ...
    // Vérifier que le trajet a déjà eu lieu
    if (trajet.date_depart > new Date()) {
      return res.status(400).json({
        message:
          "Un trajet ne peut être marqué comme terminé que si sa date de départ est passée",
      });
    }
    ...
    res.json({
      success: true,
      message:
        "Trajet marqué comme terminé. Les évaluations sont maintenant disponibles.",
      trajet,
      reservationsMisesAJour: result.modifiedCount || 0,
    });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
